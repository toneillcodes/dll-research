...
#include <windows.h>
#include <winternl.h>
#include <stdio.h>          // this really shouldn't be here but helps to demonstrate concepts
...
DWORD WINAPI AgentLoop(LPVOID lpParam) {
    ...
    while (TRUE) {
        ...
        // Standard, non-malicious workload
        SYSTEMTIME st;
        GetLocalTime(&st);
        printf("[PAYLOAD] Local System Time: %02d:%02d:%02d\n", st.wHour, st.wMinute, st.wSecond);

        // todo: mimic beacon 'Jitter' for sleep intervals
...
{
    //printf("[DEBUG] (ReflectiveLoader) ReflectiveLoader entered.\n");
...
__declspec(dllexport)
void StartReflective(void)
{
    ReflectiveLoader();   // now _ReturnAddress() is inside the DLL
}